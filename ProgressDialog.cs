...
internal sealed class ProgressDialog : Form
{
    ...
    public ProgressDialog(Form owner, string title, IEnumerable<string> phases, AppConfig config)
    {
        ...
        layout.Controls.Add(_stageBar, 0, 1);
        layout.Controls.Add(_overallText, 0, 2);
        layout.Controls.Add(_overallBar, 0, 3);
        layout.Controls.Add(_eta, 0, 4);
        layout.Controls.Add(cancel, 0, 5);
        Controls.Add(layout);

        _text.Text = title + "...";
        _overallText.Text = "Overall progress  0%";
    ...
    public void SetPhase(string name)
    {
        ...
        if (string.Equals(_phase, name, StringComparison.OrdinalIgnoreCase)) return;
        ...
        _explicitProgress = null;
        _detail = null;
        _stageBar.Style = ProgressBarStyle.Marquee;
        _stageBar.MarqueeAnimationSpeed = 28;
        _text.Text = PhaseText(name);
        UpdateOverall();
    }
    ...
    public void SetProgress(double percent, string? detail = null)
    {
        ...
        _explicitProgress = Math.Clamp(percent, 0, 100);
        _detail = string.IsNullOrWhiteSpace(detail) ? null : detail.Trim();
        _stageBar.Style = ProgressBarStyle.Blocks;
        _stageBar.Value = Math.Clamp((int)Math.Round(_explicitProgress.Value), 0, 100);
        _text.Text = $"{PhaseText(_phase)}  {_explicitProgress.Value:0}%" + (_detail is null ? string.Empty : $"   {_detail}");
        UpdateOverall();
    }
    ...
    public void Finish(bool learn = true)
    {
        ...
        if (learn && _phase is not null) LearnPhase(_phase, DateTime.UtcNow - _phaseStarted);
        _finishing = true;
        _timer.Stop();
        _stageBar.Style = ProgressBarStyle.Blocks;
        _stageBar.Value = 100;
        _overallBar.Value = 100;
        _overallText.Text = "Overall progress  100%";
    ...
    private void CompletePreviousPhase()
    {
        ...
        var oldAverage = GetAverage(_phase);
        LearnPhase(_phase, DateTime.UtcNow - _phaseStarted);
        _completedWeight += oldAverage;
    }

    private void LearnPhase(string phase, TimeSpan elapsed)
    {
        var seconds = Math.Max(0.01, elapsed.TotalSeconds);
        var previous = GetAverage(phase);
        var alpha = _config.Samples < 3 ? 0.45 : 0.22;
    ...
    {
        if (_finishing) return;
        if (_phase is not null && !_explicitProgress.HasValue)
        {
            _text.Text = PhaseText(_phase);
        }
        UpdateOverall();
    }
    ...
    private void UpdateOverall()
    {
        ...
        var total = Math.Max(0.1, _phases.Sum(GetAverage));
        var currentWeight = 0.0;
        if (_phase is not null)
        {
            var average = GetAverage(_phase);
            var fraction = _explicitProgress.HasValue
                ? _explicitProgress.Value / 100.0
                : Math.Min(0.95, (DateTime.UtcNow - _phaseStarted).TotalSeconds / Math.Max(average, 0.05));
            currentWeight = average * fraction;
        }

        var percent = Math.Clamp((_completedWeight + currentWeight) / total * 100.0, 0, 99.5);
        _overallBar.Value = Math.Clamp((int)Math.Round(percent), 0, 99);
        _overallText.Text = $"Overall progress  {percent:0}%";

        if (_config.Samples >= 3)
        {
            var remaining = Math.Max(0, total - _completedWeight - currentWeight);
            _eta.Text = $"Estimated time remaining: ~{FormatEta(remaining)}";
        }
        else
        {
            _eta.Text = "Learning typical processing times...";
        }
    }
    ...
    private static string PhaseText(string? name) => name switch
    {
        ...
        "subtitle-download" => "Downloading subtitles...",
        "subtitle-format" => "Creating subtitle output...",
        "subtitle-save" => "Saving subtitles...",
        "video-download" => "Downloading video...",
        "video-postprocess" => "Processing video...",
        "audio-download" => "Downloading audio...",
        "audio-convert" => "Converting to MP3...",
        "media-finalize" => "Finalizing output...",
        null => "Preparing...",
        _ => name.Replace('-', ' ') + "...",
    };
    ...
    {
        var time = TimeSpan.FromSeconds(Math.Max(0, Math.Round(seconds)));
