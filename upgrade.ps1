...
$UpgradeRevision = '2.22-runner-01'
$ExpectedVersion = '2.22'
...
try {
    ...
    $dotnetVersion = Invoke-NativeCapture $dotnet @('--version'); Write-UpgradeLine ("Build SDK: .NET {0}" -f $dotnetVersion.Output[0]); $toolsUpdate = Join-Path $Repo 'tools-update.ps1'; if (-not (Test-Path -LiteralPath $toolsUpdate)) { throw 'tools-update.ps1 is missing.' }; Invoke-Native (Join-Path $PSHOME 'powershell.exe') @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $toolsUpdate); foreach ($tool in @('tools\yt-dlp.exe', 'tools\ffmpeg.exe', 'tools\ffprobe.exe')) { if (-not (Test-Path -LiteralPath (Join-Path $Repo $tool))) { throw ("Required media tool is missing after update: {0}" -f $tool) } }
    Set-Phase 'MIGRATION'; foreach ($legacy in @('__pycache__', '.venv', '.pytest_cache')) { $path = Join-Path $Repo $legacy; if (Test-Path -LiteralPath $path) { Write-UpgradeLine ("Removing obsolete Python artifact: {0}" -f $legacy); Remove-Item -LiteralPath $path -Recurse -Force } }
    Set-Phase 'CONFIGURATION'; $encodedIcon = Join-Path $Repo 'assets\ytsubs.ico.b64'; $icon = Join-Path $Repo 'assets\ytsubs.ico'; if (-not (Test-Path -LiteralPath $encodedIcon)) { throw 'Missing encoded Windows icon asset.' }; $raw = [Convert]::FromBase64String((Get-Content -LiteralPath $encodedIcon -Raw)); if ($raw.Length -lt 22 -or $raw[0] -ne 0 -or $raw[1] -ne 0 -or $raw[2] -ne 1 -or $raw[3] -ne 0) { throw 'Encoded icon validation failed.' }; [IO.File]::WriteAllBytes($icon, $raw)
    Set-Phase 'BUILD'; foreach ($relative in @('obj', 'bin', 'cli\obj', 'cli\bin', 'build\publish-gui', 'build\publish-cli')) { $path = Join-Path $Repo $relative; if (Test-Path -LiteralPath $path) { Remove-Item -LiteralPath $path -Recurse -Force } }
    Invoke-Native $dotnet @('restore', (Join-Path $Repo 'YouTubeSubs.csproj')); Invoke-Native $dotnet @('restore', (Join-Path $Repo 'cli\YouTubeSubs.Cli.csproj')); Invoke-Native $dotnet @('build', (Join-Path $Repo 'YouTubeSubs.csproj'), '-c', 'Release', '--no-restore'); Invoke-Native $dotnet @('build', (Join-Path $Repo 'cli\YouTubeSubs.Cli.csproj'), '-c', 'Release', '--no-restore')
    Set-Phase 'DIST'; $guiDir = Join-Path $Repo 'build\publish-gui'; $cliDir = Join-Path $Repo 'build\publish-cli'; Invoke-Native $dotnet @('publish', (Join-Path $Repo 'YouTubeSubs.csproj'), '-c', 'Release', '-r', 'win-x64', '--self-contained', 'true', '--no-restore', '-p:PublishSingleFile=true', '-p:PublishTrimmed=false', '-o', $guiDir); Invoke-Native $dotnet @('publish', (Join-Path $Repo 'cli\YouTubeSubs.Cli.csproj'), '-c', 'Release', '-r', 'win-x64', '--self-contained', 'true', '--no-restore', '-p:PublishSingleFile=true', '-p:PublishTrimmed=false', '-o', $cliDir)
    $guiCandidate = Join-Path $guiDir 'ytsubs.exe'; $cliCandidate = Join-Path $cliDir 'ytsubs-cli.exe'; if (-not (Test-Path -LiteralPath $guiCandidate) -or -not (Test-Path -LiteralPath $cliCandidate)) { throw 'Published GUI or CLI candidate is missing.' }; $guiSubsystem = Get-PeSubsystem $guiCandidate; $cliSubsystem = Get-PeSubsystem $cliCandidate; Write-UpgradeLine ("GUI subsystem: {0}" -f $guiSubsystem); Write-UpgradeLine ("CLI subsystem: {0}" -f $cliSubsystem); if ($guiSubsystem -ne 2) { throw 'GUI candidate does not use Windows GUI subsystem 2.' }; if ($cliSubsystem -ne 3) { throw 'CLI candidate does not use Windows console subsystem 3.' }
    $expectedOutput = 'ytsubs-cli ' + $ExpectedVersion; $versionCheck = Invoke-NativeCapture $cliCandidate @('--version'); if ($versionCheck.ExitCode -ne 0 -or $versionCheck.Output.Count -eq 0 -or $versionCheck.Output[0] -ne $expectedOutput) { throw 'CLI candidate version mismatch.' }; Write-UpgradeLine ("Version validation: {0}" -f $versionCheck.Output[0])
    Set-Phase 'STOP-RUNTIME'; Stop-RunningApplication
    Set-Phase 'DEPLOY'; Copy-Item -LiteralPath $guiCandidate -Destination (Join-Path $Repo 'ytsubs.exe') -Force; Copy-Item -LiteralPath $cliCandidate -Destination (Join-Path $Repo 'ytsubs-cli.exe') -Force; $installed = Invoke-NativeCapture (Join-Path $Repo 'ytsubs-cli.exe') @('--version'); if ($installed.ExitCode -ne 0 -or $installed.Output.Count -eq 0 -or $installed.Output[0] -ne $expectedOutput) { throw 'Installed CLI validation failed.' }; Write-UpgradeLine ("Installed validation: {0}" -f $installed.Output[0])
    Set-Phase 'RESTART'; if ($WasRunning) { Start-Process -FilePath (Join-Path $Repo 'ytsubs.exe') -WorkingDirectory $Repo | Out-Null }
    Set-Phase 'COMPLETE'; if ($HadWarning) { Write-UpgradeLine ("STATUS: WARNING - YouTubeSubs {0}" -f $ExpectedVersion) Yellow } else { Write-UpgradeLine ("STATUS: SUCCESS - YouTubeSubs {0}" -f $ExpectedVersion) Green }; exit 0
}
catch { Write-UpgradeLine ("ERROR: {0}" -f $_.Exception.Message) Red; Write-UpgradeLine ("STATUS: FAILED - YouTubeSubs {0} - phase={1}" -f $ExpectedVersion, $Phase) Red; exit 1 }
