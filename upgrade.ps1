...
$UpgradeRevision = '2.14-runner-01'
...
$LogPath = Join-Path $Repo 'upgrade.log'
$Phase = 'SELF-UPDATE'
$HadWarning = $false
$WasRunning = $false
$AppProcessId = $null
...
function Invoke-Native {
    ...
    try {
        & $File @Arguments 2>&1 | ForEach-Object { Write-UpgradeLine ([string]$_) }
        $code = $LASTEXITCODE
    }
    finally {
...
function Stop-RunningApplication {
    ...
    Get-Process -Name 'ytsubs' -ErrorAction SilentlyContinue | ForEach-Object {
        try {
            if ([string]::Equals([IO.Path]::GetFullPath($_.Path), $targetFull, [StringComparison]::OrdinalIgnoreCase)) { $matches += $_ }
        }
        catch { }
    }
...
try {
    ...
    $startSha = Invoke-NativeCapture $git @('-C', $Repo, 'rev-parse', 'HEAD')
    ...
    $localSha = $local.Output[0]
    $remoteSha = $remote.Output[0]
    $baseSha = $base.Output[0]
    if ($localSha -ne $remoteSha) {
        if ($localSha -ne $baseSha) { throw ("Local branch diverged from origin/{0}; automatic destructive reset is intentionally refused." -f $Branch) }
    ...
    if ($dotnetVersion.ExitCode -ne 0) { throw 'Unable to query the .NET SDK version.' }
    ...
    Invoke-Native $powershellExe @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $toolsUpdate)
    foreach ($tool in @('tools\yt-dlp.exe', 'tools\ffmpeg.exe')) {
    ...
    foreach ($legacy in @('__pycache__', '.venv', '.pytest_cache')) {
        ...
        if (Test-Path -LiteralPath $path) {
            Write-UpgradeLine ("Removing obsolete Python artifact: {0}" -f $legacy)
    ...
    if ($versionCheck.ExitCode -ne 0 -or $versionCheck.Output.Count -eq 0 -or $versionCheck.Output[0] -ne $expectedOutput) {
        throw ("CLI candidate version mismatch. Expected '{0}'." -f $expectedOutput)
    }
    ...
    Write-UpgradeLine 'Media tools: latest stable yt-dlp + FFmpeg'
    Write-UpgradeLine 'Network repository support: exact safe.directory bootstrap when Git requires it'
    Write-UpgradeLine 'Build system: .NET 10 self-contained single-file win-x64'

    Set-Phase 'RESTART'
    if ($WasRunning) {
        $installedGui = Join-Path $Repo 'ytsubs.exe'
        Write-UpgradeLine 'YouTubeSubs was running before upgrade; starting the updated application.'
        Start-Process -FilePath $installedGui -WorkingDirectory $Repo | Out-Null
    }

    Set-Phase 'COMPLETE'
    if ($HadWarning) {
        Write-UpgradeLine 'STATUS: WARNING - phase=COMPLETE' Yellow
    }
    else {
        Write-UpgradeLine 'STATUS: SUCCESS - phase=COMPLETE' Green
    }
    exit 0
}
catch {
    Write-UpgradeLine ("ERROR: {0}" -f $_.Exception.Message) Red
    Write-UpgradeLine ("STATUS: FAILED - phase={0}" -f $Phase) Red
    exit 1
}
