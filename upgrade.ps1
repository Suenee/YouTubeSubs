...
try {
 ...
 Set-Phase 'DEPLOY'; Copy-Item $gui (Join-Path $Repo 'ytsubs.exe') -Force; Copy-Item $cli (Join-Path $Repo 'ytsubs-cli.exe') -Force; $installed=Invoke-NativeCapture (Join-Path $Repo 'ytsubs-cli.exe') @('--version'); if($installed.ExitCode -ne 0 -or $installed.Output[0] -ne $expected){throw 'Installed CLI validation failed.'}; Write-UpgradeLine ("Installed validation: {0}" -f $installed.Output[0])
