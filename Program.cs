...
internal static class Program
{
    ...
    private static int RunGui(AppConfig config, Stopwatch startup)
    {
        ...
        catch (SocketException)
        {
            ...
            try { using var sender = new UdpClient(AddressFamily.InterNetwork); sender.Send("ACTIVATE"u8.ToArray(), new IPEndPoint(IPAddress.Loopback, GuiPort)); }
        ...
        UiLayoutFix.Apply(form); UiInteractionFix.Attach(form); UiDiagnostics.Attach(form);
        ...
        _ = Task.Run(async () =>
        {
            while (!cancellation.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (Encoding.ASCII.GetString(result.Buffer) == "ACTIVATE" && !form.IsDisposed) { AppLog.Write("IPC ACTIVATE received"); form.BeginInvoke(new Action(form.ActivateFront)); }
                }
