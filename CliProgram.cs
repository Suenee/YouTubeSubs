...
{
    private const string Version = "2.08";
    ...
    private static int Main(string[] args)
    {
        ...
        AppLog.Write($"start version={Version} mode=cli args={string.Join(' ', args)}");
    ...
    private static int LaunchGui()
    {
        try
        {
            var guiPath = Path.Combine(AppContext.BaseDirectory, "ytsubs.exe");
            if (!File.Exists(guiPath))
            {
                Console.Error.WriteLine("ytsubs-cli: ytsubs.exe was not found next to ytsubs-cli.exe.");
                return 4;
            }
        ...
        {
            Console.Error.WriteLine($"ytsubs-cli: unable to start GUI: {ex.Message}");
            return 4;
        }
    }
    ...
    private static async Task<int> RunCliAsync(string[] args)
    {
        ...
        {
            Console.Error.WriteLine($"ytsubs-cli: {ex.Message}");
            return 2;
        }
        ...
        {
            Console.Error.WriteLine($"ytsubs-cli: unable to write output: {ex.Message}");
            return 5;
        }
        ...
        {
            Console.Error.WriteLine($"ytsubs-cli: unable to retrieve subtitles: {ex.Message}");
            return 4;
        }
    }
}
