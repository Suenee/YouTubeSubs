...
internal sealed class AppConfig
{
    ...
    public void Save()
    {
        ...
        {
            // Configuration persistence must never make the application unusable.
        }
    }

    private void Normalize()
    {
        if (Logging is not ("off" or "single" or "all"))
            Logging = "off";

        if (LastFormat is not ("srt" or "sub" or "txt" or "vtt"))
            LastFormat = "srt";
        ...
        foreach (var item in defaults)
            PhaseSeconds.TryAdd(item.Key, item.Value);
    }
